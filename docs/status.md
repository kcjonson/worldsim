...
**Status:** complete (merged #208, #210, #212)
...
- [x] Hydrology ponds: remove noise-flood ponds; sparse biome/precip-weighted PondNetwork2D; desert oasis via distance-to-water (#212)
- [x] Proper forests: implement `Distribution::Spaced`; grove/glade/thicket density field; denser forest trees (#212)
...
### Rust Rewrite
**Spec/Documentation:** `docs/technical/rust-rewrite-plan.md`, `docs/technical/rust-migration-scope.md`, `docs/technical/rendering/vulkan-migration.md` (PR #271)
**Dependencies:** none; supersedes remaining C++ feature work once the freeze starts
**Status:** ready (Phase 0)

**Goal:** Rewrite the codebase in Rust with Claude agents: layered, dependency-ordered, FFI-free, each layer gated on oracles against the C++ build. Faithful translation except three contract-governed redesign zones (Vulkan renderer, ECS, UI).

**Tasks:**
- [ ] Phase 0: C++ prep (WOR-466, determinism sweep, headless deterministic sim + replay oracle, automated scenario tests, untested-system coverage, deferred ECS structural changes, goldens, differential harness, freeze decision)
- [ ] Phase 1: rulebook, contracts, crate map, stress test, Vulkan/ECS/UI spikes, run infrastructure, foundation pilot + budget
- [ ] Phase 2: geometry, world, worldgen-cli in Rust (worldHash + byte-equal .wsplanet)
- [ ] Phase 3: engine core headless (replay equality, scenario tests over the HTTP API)
- [ ] Phase 4: Vulkan renderer, UI, planet-view, apps (goldens, UI tree parity, perf)
- [ ] Phase 5: parity, Mac smoke, delete the C++ tree, rewrite CI and docs
...
**Tasks:**
- [x] A/B/C: Simulation-area nav LOD — viewport-anchored navmesh build (forested world builds <1s), pan/zoom tracking (capped 64m, rebuild on in-area placement), off-area beeline seam (#217)
- [x] D1: Rect collision primitive — drop Circle; rect-or-polygon; XML `<rect>` + Lua `asset:setCollisionRect` (eager capture at load); trees emit trunk rects (#220)
