...
ColonistListItem::ColonistListItem(const Args& args)
	...
	backgroundHandle = addChild(UI::Rectangle(UI::Rectangle::Args{
		...
		.size = size,
		.style = {
			.fill = selected ? UI::Theme::Colors::selectionBackground
							 : UI::Theme::Colors::cardBackground,
			.border = Foundation::BorderStyle{
				.color = selected ? UI::Theme::Colors::selectionBorder
	...
	nameTextHandle = addChild(UI::Text(UI::Text::Args{
	...
	moodBarHandle = addChild(UI::Rectangle(UI::Rectangle::Args{
...
void ColonistListItem::render() {
	...
	// Update child positions relative to content position
	if (auto* bg = getChild<UI::Rectangle>(backgroundHandle)) {
		bg->position = contentPos;
	}
	if (auto* nameText = getChild<UI::Text>(nameTextHandle)) {
		float textX = contentPos.x + kPortraitSize + kPortraitMargin +
					  (size.x - kPortraitSize - kPortraitMargin) / 2.0F;
...
bool ColonistListItem::containsPoint(Foundation::Vec2 point) const {
	Foundation::Vec2 contentPos = getContentPosition();
	return point.x >= contentPos.x && point.x <= contentPos.x + size.x &&
		   point.y >= contentPos.y && point.y <= contentPos.y + size.y;
}
...
void ColonistListItem::updateBackgroundStyle() {
	if (auto* bg = getChild<UI::Rectangle>(backgroundHandle)) {
		bg->style.fill = selected ? UI::Theme::Colors::selectionBackground
								  : UI::Theme::Colors::cardBackground;
		if (bg->style.border) {
			bg->style.border->color = selected ? UI::Theme::Colors::selectionBorder
											   : UI::Theme::Colors::cardBorder;
		}
	}
}
