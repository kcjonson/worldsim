...
void TestAlignment() {
	...
	struct Aligned1 { uint8_t data; };
	struct Aligned4 { uint32_t data; };
	struct Aligned8 { uint64_t data; };
	struct Aligned16 { double data[2]; };
...
static bool s_registered = []() {
	engine::SceneManager::Get().RegisterScene("arena", []() {
		return std::make_unique<ArenaScene>();
	});
	return true;
}();
