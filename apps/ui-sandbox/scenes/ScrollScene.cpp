...
class ScrollScene : public engine::IScene {
  public:
	...
	void onEnter() override {
		...
		scrollContainer1 = std::make_unique<ScrollContainer>(ScrollContainer::Args{
			.position = {50.0F, 95.0F},
			.size = {200.0F, 200.0F},
			.id = "scroll_1"});

		// Create a layout container as the scrollable content
		auto content1 = LayoutContainer(LayoutContainer::Args{
			.position = {0.0F, 0.0F},
		...
		for (int i = 0; i < 15; ++i) {
			Color itemColor = (i % 2 == 0)
								  ? Color(0.25F, 0.35F, 0.45F, 1.0F)
								  : Color(0.30F, 0.40F, 0.50F, 1.0F);

			content1.addChild(Rectangle(Rectangle::Args{
				.size = {180.0F, 30.0F},
				.style = {
					.fill = itemColor,
		...
		scrollContainer2 = std::make_unique<ScrollContainer>(ScrollContainer::Args{
			.position = {300.0F, 95.0F},
			.size = {220.0F, 200.0F},
			.id = "scroll_2"});

		auto content2 = LayoutContainer(LayoutContainer::Args{
			.position = {0.0F, 0.0F},
			.size = {212.0F, 0.0F},
			.direction = Direction::Vertical,
		...
		for (int i = 0; i < 12; ++i) {
			content2.addChild(Button(Button::Args{
