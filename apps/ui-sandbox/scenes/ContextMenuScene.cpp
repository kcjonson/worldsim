	  ...
	  public:
		...
		void onEnter() override {
			...
			instructions1 = std::make_unique<Text>(Text::Args{
				...
				.text = "Right-click (or Ctrl+Click on Mac) anywhere to open a context menu",
			...
			zone1Label = std::make_unique<Text>(Text::Args{
				.position = {75.0F, 220.0F}, .text = "Edit Zone", .style = {.color = Color::white(), .fontSize = 12.0F}, .id = "zone1_label"
			});
		...
		bool handleInput(UI::InputEvent& event) override {
			...
			if (event.type == InputEvent::Type::MouseDown) {
				// Check for right-click OR Ctrl+Click (Mac alternative)
				// GLFW_MOD_CONTROL = 0x0002
				constexpr int kCtrlModifier = 0x0002;
				bool		  isRightClick = (event.button == engine::MouseButton::Right);
				bool		  isCtrlClick = (event.button == engine::MouseButton::Left) && ((event.modifiers & kCtrlModifier) != 0);
		...
		void render() override {
			...
			Renderer::Primitives::drawRect(
				Renderer::Primitives::RectArgs{
					.bounds = zone1Rect,
					.style =
						{.fill = Color(0.2F, 0.25F, 0.3F, 1.0F),
			...
			Renderer::Primitives::drawRect(
				Renderer::Primitives::RectArgs{
					.bounds = zone2Rect,
					.style =
						{.fill = Color(0.25F, 0.2F, 0.3F, 1.0F),
			...
			Renderer::Primitives::drawRect(
				Renderer::Primitives::RectArgs{
					.bounds = zone3Rect,
					.style =
						{.fill = Color(0.3F, 0.2F, 0.25F, 1.0F),
