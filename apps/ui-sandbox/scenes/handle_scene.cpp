...
void TestBasicAllocation() {
	...
	LOG_INFO(UI, "  Handle 1: index=%d, gen=%d, valid=%s",
		handle1.GetIndex(), handle1.GetGeneration(), handle1.IsValid() ? "true" : "false");
	LOG_INFO(UI, "  Handle 2: index=%d, gen=%d, valid=%s",
		handle2.GetIndex(), handle2.GetGeneration(), handle2.IsValid() ? "true" : "false");
	LOG_INFO(UI, "  Handle 3: index=%d, gen=%d, valid=%s",
		handle3.GetIndex(), handle3.GetGeneration(), handle3.IsValid() ? "true" : "false");
...
void TestFreeListReuse() {
	...
	LOG_INFO(UI, "  New handle 1: index=%d, gen=%d (should reuse index 3, gen 1)",
		newHandle1.GetIndex(), newHandle1.GetGeneration());
	LOG_INFO(UI, "  New handle 2: index=%d, gen=%d (should reuse index 2, gen 1)",
		newHandle2.GetIndex(), newHandle2.GetGeneration());
...
void TestStaleHandles() {
	...
	LOG_INFO(UI, "");
	LOG_INFO(UI, "Allocated new handle in same slot: index=%d, gen=%d",
		newHandle.GetIndex(), newHandle.GetGeneration());
	...
	LOG_INFO(UI, "Accessing with old handle after realloc: %s",
		staleRes ? "FAIL - got resource!" : "PASS - returned null");
...
void TestHandleValidation() {
	...
	LOG_INFO(UI, "Invalid handle: value=0x%08x, valid=%s",
		invalidHandle.value, invalidHandle.IsValid() ? "true" : "false");
	...
	LOG_INFO(UI, "Get with out-of-range index (9999): %s",
		res ? "FAIL - got resource!" : "PASS - returned null");
...
static bool s_registered = []() {
	engine::SceneManager::Get().RegisterScene("handles", []() {
		return std::make_unique<HandleScene>();
	});
	return true;
}();
