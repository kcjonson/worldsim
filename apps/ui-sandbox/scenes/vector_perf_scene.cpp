...
class VectorPerfScene : public engine::IScene {
  public:
	...
	void Render() override {
		...
		for (const auto& star : m_stars) {
			if (!star.mesh.vertices.empty()) {
				Renderer::Primitives::DrawTriangles({.vertices = star.mesh.vertices.data(),
		...
		Renderer::Primitives::DrawRect({.bounds = {10, 10, 150, 30},
										.style = {.fill = Color(0.0f, 0.0f, 0.0f, 0.7f)}});

		// Draw performance stats (top-left, below FPS)
		char statsText[256];
		snprintf(statsText, sizeof(statsText), "Stars: %zu | Triangles: %zu\nRender: %.2fms", m_stars.size(),
				 CalculateTotalTriangles(), renderMs);
	...
	std::string ExportState() override {
		char buf[256];
		snprintf(buf, sizeof(buf), R"({"stars": %zu, "fps": %.1f, "renderMs": %.2f})", m_stars.size(), m_fps,
				 m_lastRenderTime);
  ...
  private:
	...
	renderer::VectorPath CreateStarPath(const Foundation::Vec2& center, float outerRadius, float innerRadius) {
		...
		for (int i = 0; i < numPoints * 2; ++i) {
			float angle =
				(i * std::numbers::pi_v<float> / numPoints) - std::numbers::pi_v<float> / 2.0f; // Start at top
