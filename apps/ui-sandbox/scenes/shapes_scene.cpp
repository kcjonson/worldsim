...
public:
	...
	void Render() override {
		...
		Renderer::Primitives::DrawRect({
			.bounds = {300, 50, 200, 100},
			.style = {.fill = Color::Green()},
			.id = "green_rect"
		});

		Renderer::Primitives::DrawRect({
			.bounds = {550, 50, 200, 100},
		...
		Renderer::Primitives::DrawRect({
			...
			.style = {
				...
				.border = BorderStyle{.color = Color::Yellow(), .width = 3.0f}
			},
			.id = "yellow_border"
		});
		...
		Renderer::Primitives::DrawRect({
			...
			.style = {
				.fill = Color::Transparent(),
				.border = BorderStyle{.color = Color::Cyan(), .width = 3.0f}
			},
			.id = "cyan_border"
		});
		...
		for (int y = 0; y < 10; y++) {
			for (int x = 0; x < 10; x++) {
				...
				Renderer::Primitives::DrawRect({
					.bounds = {50.0f + x * 25.0f, 350.0f + y * 20.0f, 20.0f, 15.0f},
					.style = {.fill = color}
				});
			}
		}
...
static bool s_registered = []() {
	engine::SceneManager::Get().RegisterScene("shapes", []() {
		return std::make_unique<ShapesScene>();
	});
	return true;
}();
