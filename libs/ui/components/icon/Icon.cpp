...
void Icon::rebuildMesh() {
	...
	// Scale vertices to fit icon size
	scaleVertices();
}

void Icon::scaleVertices() {
	if (vertices.empty() || (originalWidth == 0.0F && originalHeight == 0.0F)) {
		return;
	}
	...
	// Reload and rescale vertices from original mesh
	// (We can't easily undo previous scaling, so reload is cleaner)
	// For performance, we could cache the original vertices, but for now
	// we just recalculate from the current vertices assuming they're at original scale

	// Actually, let's reload the mesh to get fresh vertices at original scale
	if (meshDirty || vertices.empty()) {
		return; // Will be handled by rebuildMesh
	}

	// Reload the SVG to get original-scale vertices
	std::vector<renderer::LoadedSVGShape> shapes;
	if (!renderer::loadSVG(svgPath, 0.5F, shapes) || shapes.empty()) {
		return;
	}
	...
	for (const auto& shape : shapes) {
		for (const auto& path : shape.paths) {
			...
			if (tessellator.Tessellate(path, mesh)) {
				...
				// Add scaled and offset vertices
				for (const auto& v : mesh.vertices) {
