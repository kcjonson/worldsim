...
Chunk::Chunk(ChunkCoordinate coord, ChunkSampleResult biomeData, uint64_t worldSeed)
	: m_coord(coord), m_biomeData(std::move(biomeData)), m_worldSeed(worldSeed) {
	touch();
}
...
GroundCover Chunk::selectGroundCover(Biome biome, uint32_t hash) const {
	// Use hash to add variation within biome
	float variation = static_cast<float>(hash % 1000) / 1000.0F;

	switch (biome) {
		case Biome::Grassland:
			return (variation < 0.9F) ? GroundCover::Grass : GroundCover::Dirt;

		case Biome::Forest:
			return (variation < 0.85F) ? GroundCover::Grass : GroundCover::Dirt;

		case Biome::Desert:
			return (variation < 0.8F) ? GroundCover::Sand : GroundCover::Rock;

		case Biome::Tundra:
			return (variation < 0.6F) ? GroundCover::Snow : GroundCover::Rock;

		case Biome::Wetland:
			return (variation < 0.7F) ? GroundCover::Water : GroundCover::Grass;
		...
		case Biome::Mountain:
			...
			return GroundCover::Dirt;

		case Biome::Beach:
			return (variation < 0.9F) ? GroundCover::Sand : GroundCover::Rock;
