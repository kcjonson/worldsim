...
struct EvaluatedOption {
	...
	[[nodiscard]] bool isActionable() const {
		return status == OptionStatus::Selected || status == OptionStatus::Available;
	}
};
...
struct DecisionTrace {
	...
	[[nodiscard]] size_t displayCount() const {
		return std::min(options.size(), kMaxDisplayedOptions);
	}
};
