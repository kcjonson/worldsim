...
namespace ecs {
	...
	struct NeedsComponent {
		...
		/// Access need by type
		[[nodiscard]] Need& get(NeedType type) { return needs[static_cast<size_t>(type)]; }

		[[nodiscard]] const Need& get(NeedType type) const { return needs[static_cast<size_t>(type)]; }
