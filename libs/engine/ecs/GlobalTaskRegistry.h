...
namespace ecs {
	...
	struct GlobalTask {
		...
		EntityID targetEntity;		 // Reserved for future ECS-entity tasks (currently unused; world tasks use worldEntityKey)
		uint64_t worldEntityKey = 0; // Primary identity for world-entity-backed tasks (hash key from Memory)
		...
		// NOTE: Reserved for Phase 5+ task chains. Not used in current Phase 2 implementation.
		...
		// NOTE: Reserved for Phase 5 implementation. Not used in current Phase 2.
	...
	/// from the main game thread only. The game's ECS systems run sequentially on
	/// the main thread, so no synchronization is needed for current usage.
