...
namespace ecs {
		...
		if (action.hasCraftingEffect()) {
			...
			for (const auto& [itemName, count] : craftEff.inputs) {
				...
				if (removed < count) {
					LOG_WARNING(
						Engine,
						"[Action] Craft failed to consume %u x %s (only had %u)",
						count,
						itemName.c_str(),
						removed
					);
			...
			if (workQueue != nullptr) {
				...
				if (job != nullptr && job->recipeDefName == craftEff.recipeDefName) {
					...
					LOG_INFO(
						Engine,
						"[Action] Updated WorkQueue: %s %u/%u complete",
						job->recipeDefName.c_str(),
						job->completed,
						job->quantity
					);
	...
	void ActionSystem::startCraftAction(
		Task& task,
		Action& action,
		const Inventory& inventory
	) {
		...
		for (const auto& input : recipe->inputs) {
			if (!inventory.hasQuantity(input.defName, input.count)) {
				LOG_WARNING(
					Engine,
					"[Action] Cannot craft %s - missing %u x %s",
					recipe->label.c_str(),
					input.count,
					input.defName.c_str()
				);
		...
		LOG_DEBUG(
			Engine,
			"[Action] Starting Craft action for %s (%.1fs duration)",
			recipe->label.c_str(),
			action.duration
		);
	}

	void ActionSystem::startGatherAction(
		Task& task,
		Action& action,
		const Position& position,
		const Memory& memory
		...
		for (const auto& [key, entity] : memory.knownWorldEntities) {
			...
			if (registry.hasCapability(entity.defNameId, engine::assets::CapabilityType::Carryable)) {
				...
				if (def != nullptr && def->capabilities.carryable.has_value()) {
					...
					LOG_DEBUG(
						Engine,
						"[Action] Starting Pickup action for %s (qty %u)",
						defName.c_str(),
						carryableCap.quantity
					);
					return;
				}
			}
