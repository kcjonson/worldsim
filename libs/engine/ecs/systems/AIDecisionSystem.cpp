...
[[nodiscard]] engine::assets::CapabilityType needToCapability(NeedType need) {
	switch (need) {
		case NeedType::Hunger: return engine::assets::CapabilityType::Edible;
...
bool AIDecisionSystem::evaluateCriticalNeeds(
	...
	// Check all needs for critical state (< 10%)
	for (size_t i = 0; i < static_cast<size_t>(NeedType::Count); ++i) {
		...
		if (nearest.has_value()) {
			...
			task.needToFulfill = needType;
			task.targetPosition = nearest->position;
			task.reason = std::string(needTypeName(needType)) + " CRITICAL at " +
						  std::to_string(static_cast<int>(need.value)) + "%";
			return true;
		}

		// For Energy and Bladder, use ground as fallback (current position)
		if (needType == NeedType::Energy || needType == NeedType::Bladder) {
			task.type = TaskType::FulfillNeed;
			task.needToFulfill = needType;
			task.targetPosition = position.value; // Use current position (ground)
			task.reason = std::string(needTypeName(needType)) + " CRITICAL - using ground at " +
						  std::to_string(static_cast<int>(need.value)) + "%";
			return true;
		}
	}
...
bool AIDecisionSystem::evaluateActionableNeeds(
	...
	if (nearest.has_value()) {
		...
		task.targetPosition = nearest->position;
		task.reason = std::string(needTypeName(urgentNeed)) + " at " +
					  std::to_string(static_cast<int>(need.value)) + "%";
		return true;
	}
	...
	if (urgentNeed == NeedType::Energy || urgentNeed == NeedType::Bladder) {
		...
		task.needToFulfill = urgentNeed;
		task.targetPosition = position.value; // Use current position (ground)
		task.reason = std::string(needTypeName(urgentNeed)) + " - using ground at " +
					  std::to_string(static_cast<int>(need.value)) + "%";
		return true;
	}
...
void AIDecisionSystem::assignWander(
	EntityID /*entity*/,
	Task& task,
	const Position& position) {
