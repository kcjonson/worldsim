		...
		if (m_chunkManager != nullptr && m_waterTileDefNameId != 0) {
			for (int32_t cy = chunkMinY; cy <= chunkMaxY; ++cy) {
				for (int32_t cx = chunkMinX; cx <= chunkMaxX; ++cx) {
					...
					int tileMaxX = std::min(static_cast<int>(engine::world::kChunkSize) - 1,
											static_cast<int>(std::floor(maxX - origin.x)));
					int tileMinY = std::max(0, static_cast<int>(std::floor(minY - origin.y)));
					int tileMaxY = std::min(static_cast<int>(engine::world::kChunkSize) - 1,
											static_cast<int>(std::floor(maxY - origin.y)));
							...
							if (tile.surface == engine::world::Surface::Water) {
								...
								for (const auto& offset : kNeighborOffsets) {
									...
									engine::world::ChunkCoordinate neighborChunkCoord{
										static_cast<int32_t>(std::floor(neighborWorldX / static_cast<float>(engine::world::kChunkSize))),
									...
									neighborTileX = std::max(0, std::min(neighborTileX, static_cast<int>(engine::world::kChunkSize) - 1));
									neighborTileY = std::max(0, std::min(neighborTileY, static_cast<int>(engine::world::kChunkSize) - 1));

									auto neighborTile = neighborChunk->getTile(
										static_cast<uint16_t>(neighborTileX),
										static_cast<uint16_t>(neighborTileY));
