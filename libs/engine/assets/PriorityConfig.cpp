...
int16_t PriorityConfig::userPriorityToBase(uint8_t userPriority) const {
    ...
    if (userPriority < 1) userPriority = 1;
    if (userPriority > 9) userPriority = 9;
...
int16_t PriorityConfig::calculateDistanceBonus(float distance) const {
    ...
    float bonus = static_cast<float>(m_distance.maxBonus) -
                  normalized * static_cast<float>(m_distance.maxBonus + m_distance.maxPenalty);
...
void PriorityConfig::parseCategoryOrder(const void* nodePtr) {
    ...
    std::sort(categories.begin(), categories.end(),
              [](const CategoryTier& a, const CategoryTier& b) { return a.tier < b.tier; });
