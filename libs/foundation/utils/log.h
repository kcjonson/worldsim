...
namespace foundation {
	...
	enum class LogLevel : std::uint8_t { // NOLINT(performance-enum-size) Debug, Info, Warning, Error };
		...
		enum class LogCategory :
			std::uint8_t{// NOLINT(performance-enum-size) Renderer, Physics, Audio, Network, Game, World, UI, Engine, Foundation, Count };

						 class Logger{
							 public : static void Initialize(); static void Shutdown();
							 ...
							 private : static LogLevel s_levels[static_cast<int>(LogCategory::Count)];
